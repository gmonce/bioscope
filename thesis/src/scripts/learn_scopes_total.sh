...
# learn_scopes_total.sh {$WORKING_DIR | $WORKING_DIRD} $RUN $RUNX
...
learn_model=1 # Evaluar los scopes 
gen_files=1 # Generar archivos de entrenamiento
...
# Toma BIOSCOPE_TRAIN_SCOPE y genera trainx.data.total
# Toma BIOSCOPE_TEST_SCOPE y genera testx.data.total
...
if [ $learn_model = 1 ]; then
	...
	echo "Prediction results" > $WORKING_DIR/logs/log.total.$RUN.$RUNX
	echo "------------------" >> $WORKING_DIR/logs/log.total.$RUN.$RUNX
	...
	python $SRC/scripts/insert_scope_errors.py $WORKING_DIR $RUNX Y
	...
	echo "Evaluating scope detection on testing corpus using original hedge cue..." >> $WORKING_DIR/logs/log.total.$RUN.$RUNX
	java -jar $SRC/scorer_task2_cue.jar $WORKING_DIR/crf_corpus/scope/$RUN.$RUNX/gold_standard_total.xml $WORKING_DIR/crf_corpus/scope/$RUN.$RUNX/guessed_scope_total.xml >> $WORKING_DIR/logs/log.total.$RUN.$RUNX
	...
	echo " ">> $WORKING_DIR/logs/log.total.$RUN.$RUNX
	echo "Evaluating scope detection on testing corpus with guessed hedge cue..." >> $WORKING_DIR/logs/log.total.$RUN.$RUNX
	java -jar $SRC/scorer_task2_cue.jar $WORKING_DIR/crf_corpus/scope/$RUN.$RUNX/gold_standard_total.xml $WORKING_DIR/crf_corpus/scope/$RUN.$RUNX/guessed_hc_and_scope_total.xml >> $WORKING_DIR/logs/log.total.$RUN.$RUNX
