WORKING_DIR=$1 
RUN=$2
echo " "> $WORKING_DIR/logs/cv.$RUN.curve.log
...
do
	...
	cat $WORKING_DIR/crf_corpus/hc/$RUN/test.total.data.crf_results | egrep -v '#' | sed 's/\/0\.[0-9]*$//' | ./conlleval.pl -d '\t' | grep 'accuracy'   >> $WORKING_DIR/logs/cv.$RUN.curve.log
