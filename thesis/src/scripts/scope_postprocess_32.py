...
def apply_morante2009_rules(sentence):
	...
	for token in sentence:
		...
		hc_gparent_pos=token[-9]
		hc_parent_pos=token[-10]
	...
	if total_F==0 and total_L==1:
		if token[-10]=='ADJP' and token[-9]=='VP' and hc_ggparent_scope_end==first_L:
	...
	if pos_hc=='MD' and token[-11]=='Y' and hc_gparent_pos in ('S','SBAR'):
		print sentence[0][1],":Regla MD y voz pasiva, con abuelo"
		first=hc_gparent_scope_start
		last=hc_gparent_scope_end
	elif pos_hc=='MD' and token[-11]=='Y' and hc_ggparent_pos in ('S','SBAR'):
	...
	if sentence[0][1]=='S836.8':
		print sentence[hc_pos-1][-6]
	if pos_hc=='VBN' and sentence[hc_pos-1][-6]=='be' and hc_gparent_pos in ('S','SBAR'):
		print sentence[0][1],":Regla VBN y voz pasiva, con abuelo"
		first=hc_gparent_scope_start
		last=hc_gparent_scope_end
	elif pos_hc=='VBN' and sentence[hc_pos-1][-6]=='be' and hc_ggparent_pos in ('S','SBAR'):
