...
guessed_hedge_cues=[]
for row in c:
	c1.execute('select * from bioscope_test  where document_id=? and sentence_id=? and hedge_cue<>"O" order by token_num',[row['document_id'],row['sentence_id']])
	# Recorro los tokens de las hedge cues (puede haber más de una)
	for t in c1:
		if t['hedge_cue'] == 'B-SPECCUE':
			# Armo la hedge cue
			c2.execute('select * from bioscope_test where document_id=? and sentence_id=? and token_num>? and hedge_cue="I-SPECCUE"', [row['document_id'],row['sentence_id'],t['token_num']])
			hedge_cue=t['word']+' '
			for row2 in c2:
				hedge_cue+=row2['word']+' '
		...
		elif t['hedge_cue']=='D-SPECCUE':
			hedge_cue=t['word']
			# Inserto en la lista de HC
			hedge_cues.append([row['document_id'],row['sentence_id'],t['token_num'],hedge_cue])

				
	c1.execute('select * from bioscope_test where document_id=? and sentence_id=? and guessed_hedge_cue<>"O" order by token_num',[row['document_id'],row['sentence_id']])
	for t in c1:
		if t['guessed_hedge_cue'] == 'B-SPECCUE':
			# Armo la hedge cue
			c2.execute('select * from bioscope_test where document_id=? and sentence_id=? and token_num>? and hedge_cue="I-SPECCUE"', [row['document_id'],row['sentence_id'],t['token_num']])
			hedge_cue=t['word']+' '
			for row2 in c2:
...
for row in c:
	c1.execute('select * from bioscope_train where document_id=? and sentence_id=? and hedge_cue<>"O" order by token_num',[row['document_id'],row['sentence_id']])
	for t in c1:
		if t['hedge_cue'] == 'B-SPECCUE':
			c2.execute('select * from bioscope_train where document_id=? and sentence_id=? and token_num>? and hedge_cue="I-SPECCUE"', [row['document_id'],row['sentence_id'],t['token_num']])
			hedge_cue=t['word']+' '
			for row2 in c2:
				hedge_cue+=row2['word']+' '
			hedge_cue=hedge_cue[:-1]
...
#print "Hedge cues:",len(hedge_cues) 
#print "Guessed:",len(guessed_hedge_cues)
